...
contract SimpleStorage{
    ...
    // (string => uint256) defines what the mapping does - maps a string to a number, in 
    // this case name to fav number 
    ...
    // eg. People public person = People({favNumber: 13, name: "Me"});
    ...
    // or a number in the brackets specifies how large the array should be, eg. [3] creates 
    ...
    // this function modifies the state of the blockchain - so is a transaction - so costs 
    // currency
    ...
    // eg. it reads whatever is stored in favNumber but can not change/modify or update the 
    // blockchain
    function retrieve () public view returns(uint256){
        return favNumber;
    }

    // pure keyword does not allow modification of the state OR reading from the state
    // so what is it for?
    // eg. some mathematical operation you need to re-use regularly
    //eg.
    //function add() public pure returns (uint256){
    //    return (1 + 1);
    //}

    // pure and view keywords do not spend any currency as they do not modify the state of 
    // the blockchain
    // currency is only spent when making a transaction - transactions modify the state of 
    // the blockchain

    // calldata, memory, storage
    // Data location must be memory or calldata for parameter in function
    // storage variables exist outside the executing function - permanent variables that can 
    // be modified.
    // calldata and memory keywords mean that the variable (is a function parameter a 
    // variable?) will only exist temporarily.
    // In this case the variable _name is only needed when being used by the function during 
    // the transaction the function defines and is not needed any more after the function 
    ...
    // Data location can only be (and, seemingly, must be) specified for array, struct or 
    // mapping data types
    // a string is actually an array of bytes, therefore data location must be specified for 
    // variables of string type
    // This is not necessary (or possible) for uint256 type
